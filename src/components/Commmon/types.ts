...
export interface IAppTagsMapper {
  name: string;
  color: string;
}

export interface IHeaderAiAgentMetaData {
  aiModalLogo: string;
  aiModalName: string;
  tags: IAppTagsMapper[];
}
...
export type TKnowledgeType = "files" | "text" | "website" | "q-a";
...
export type InputConfig = {
  ...
  label?: string;
  placeholder?: string;
  options?: { label: string; value: string; id?: string | number; identity?: string; name?: string }[];
  min?: number;
  max?: number;
  step?: number;
...
export interface IChatThreadDetail {
  id: number;
  uuid: string;
